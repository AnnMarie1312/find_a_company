        ...
        try:
            ...
            for item in rezultati.get('items', []):
                ...
                try:
                    ...
                    firme.append({'url': url, 'naslov': naslov, 'emaili': emaili})
...
if __name__ == "__main__":
    ...
    broj_rezultata = 50  # Promijenite ovo u željeni broj rezultata
    firme = pretrazi_firme(kljucna_rijec, broj_rezultata)
    
    for firma in firme:
        print(f"URL: {firma['url']}")
        print(f"Naslov: {firma['naslov']}")
        print(f"E-pošta: {', '.join(firma['emaili']) if firma['emaili'] else 'Nema dostupnih e-adresa'}")
        print("-" * 40)
